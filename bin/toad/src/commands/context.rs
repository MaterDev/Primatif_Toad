...
use anyhow::Result;
use colored::*;
use toad_core::{ProjectAtlas, Workspace};
...
pub fn handle(
    ...
    if synthesis {
        ...
        if let Some(p_name) = project_name {
            ...
            println!(
                "{} Specify a primary project with --project to compare against.",
                "ERROR:".red().bold()
            );
        }
        ...
        println!(
            "{} Provide a --task, --inspire query, or --compare to generate a report.",
            "INFO:".blue().bold()
        );
    }
...
fn generate_comparison_report(projects: &[toad_core::ProjectDetail], a: &str, b: &str) {
    ...
    match (proj_a, proj_b) {
        (Some(p1), Some(p2)) => {
            ...
            println!(
                "Compatibility Score: {}%",
                match preflight.compatibility_score {
        ...
        _ => {
            println!(
                "{} One or both projects not found in registry.",
                "ERROR:".red().bold()
            );
        }
    }
...
fn generate_task_report(
    projects: &[toad_core::ProjectDetail],
    atlas: &ProjectAtlas,
    task: &str,
    target_project: Option<&str>,
) {
    println!(
        "{} Building tailored context for task: {}",
        "ANALYZING:".blue().bold(),
        task
    );
    ...
    if let Some(name) = target_project {
        if let Some(p) = projects.iter().find(|p| p.name == name) {
            ...
            for (p_name, dna) in &atlas.dna_map {
                ...
                let matches_task = dna
                    ...
                    || dna
                        .structural_patterns
                        .iter()
                        .any(|sp| task_lower.contains(&sp.to_lowercase()));
            ...
            if !relevant_examples.is_empty() {
                println!(
                    "\n{} These projects already implement similar patterns:",
                    "INSPIRATION:".yellow().bold()
                );
        ...
        println!(
            "{} Specify a project with --project to get deeper insights.",
            "TIP:".yellow().bold()
        );
    }
}

fn generate_inspiration_report(
    projects: &[toad_core::ProjectDetail],
    _atlas: &ProjectAtlas,
    query: &str,
) {
    println!(
        "{} Searching for structural patterns matching: {}",
        "QUERY:".blue().bold(),
        query
    );
    ...
    for p in projects {
        ...
        let matches_dna = dna
            ...
            || dna
                .structural_patterns
                .iter()
                .any(|sp| sp.to_lowercase().contains(&query_lower));
    ...
    if matches.is_empty() {
        ...
        println!(
            "\n{} Matches found in the following projects:",
            "SUCCESS:".green().bold()
        );
        for p in matches {
            ...
            if !p.dna.capabilities.is_empty() {
                println!(
                    "    Capabilities: {}",
                    p.dna.capabilities.join(", ").dimmed()
                );
            }
        }
    }
