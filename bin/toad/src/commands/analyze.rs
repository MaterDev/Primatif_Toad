...
{}", "--- INSIGHTS & RECOMMENDATIONS ---".yellow().bold());
                    for insight in insights {
                        println!("{} {} ({})", "💡".yellow(), insight.title.bold(), insight.severity.dimmed());
...
{} {}:", "📦".blue(), p.name.bold());
                    for sub in &p.submodules {
                        let status = if sub.initialized { "✅ Aligned" } else { "⚠️ Uninitialized" };
...
{}", "Critical Path (Most Depended Upon):".yellow());
    for (i, name) in graph.critical_path.iter().take(5).enumerate() {
        if let Some(node) = graph.nodes.get(name) {
             println!("  {}. {} ({} dependents)", i+1, name.bold(), node.dependents.len());
        }
    }
...
{} {}:", "🚀".green(), name.bold());
    ...
    println!("  Churn:   +{} -{} lines", v.lines_added.to_string().green(), v.lines_removed.to_string().red());
...
{} {}:", "⚠️".yellow(), name.bold());
    ...
    println!("  Comments:   {} TODOs, {} FIXMEs, {} HACKs", 
...
fn render_health(name: &str, h: &toad_core::HealthScore) {
    let status_color = if h.total > 80 { "green" } else if h.total > 50 { "yellow" } else { "red" };
    println!("
{} {}: {}/100", "🏥".color(status_color), name.bold(), h.total.to_string().color(status_color).bold());
    println!("  VCS:   {}/20  Tests: {}/20  Docs: {}/15", h.vcs_cleanliness, h.test_coverage, h.documentation);
    println!("  Pulse: {}/15  Deps:  {}/15  Qual: {}/15", h.activity, h.dependencies, h.code_quality);
}
