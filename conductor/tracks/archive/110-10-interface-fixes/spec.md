...
Fix compilation errors, path-resolution bugs, and UI glitches in the primary binaries (`toad`, `toad-mcp`) and the parent Hub configuration.
...
2. **toad CLI Accuracy:**
   - Fix `ggit` path resolution bug (correctly join submodule paths relative to their parent repo).
