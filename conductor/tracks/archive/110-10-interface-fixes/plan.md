...
- [ ] **IF-3: Fix ggit Path Building**
  - Update `ggit` handlers to use `p.path.join(&sub.path)` instead of the project-relative path joined to `projects_dir`.
...
- [ ] **IF-5: Readable CHANGELOG**
  - Re-hydrate root `CHANGELOG.md` with high-level summaries from the v1.1.0 cycle.
- [ ] **IF-6: Version Alignment**
  - Ensure all submodule pointers are aligned to their `dev` branches and versions match.
