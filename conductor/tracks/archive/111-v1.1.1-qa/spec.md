...
## Overview
Perform a comprehensive QA audit of the Primatif_Toad ecosystem (CLI + MCP) to ensure stability, usability, and architectural integrity before the next major release. This track focuses on empirical validation, documentation of issues, and establishing a baseline for automated regression testing.

## Goals
1. **Validate Core CLI** - Comprehensive testing of all subcommands.
2. **Verify MCP Integration** - Ensure all MCP tools return high-fidelity, accurate data.
3. **Audit Ecosystem Health** - Use Toad's own analysis tools to find structural weaknesses.
4. **Safety Verification** - Test guardrails and destructive operations in a controlled sandbox.
